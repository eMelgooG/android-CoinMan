...
public class CoinGame extends ApplicationAdapter {
	...
	if(gameState==1){
		//GAME IS LIVE
		if(bombCount<250) {               //every 250 frames we makeBomb
			bombCount++;
		} else {
			bombCount = 0;
			makeBomb();
		...
		bombRectangle.clear();
		for(int i = 0; i<bombXs.size();i++) {                            //make the bomb move
		...
		if(cointCount<100) {               //every 100 frames we makeCoin
			cointCount++;
		} else {
			cointCount = 0;
			makeCoin();
		...
		coinRectangle.clear();
		for(int i = 0; i<coinXs.size();i++) {                            //make the coin move
